...
class _ObjavaOglasaState extends State<ObjavaOglasa> {
  ...
  Future<void> _objaviOglas() async {
    ...
    try {
      ...
      if (mounted) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('Oglas uspješno objavljen!')),
        );
      }
    } catch (e) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text('Greška pri objavi: $e')),
      );
    } finally {
        ...
        child: SingleChildScrollView(
          ...
          child: Column(
            children: [
              ...
              const Text(
                'Ovdje možete objaviti oglas za kojigod posao vam je potrebna pomoć!',
                textAlign: TextAlign.center,
              ...
              Row(
                ...
                children: [
                  Container(
                    ...
                    child: const Center(child: Text("Slika\nposla", textAlign: TextAlign.center, style: TextStyle(color: Colors.white, fontSize: 12))),
                  ),
                  const SizedBox(width: 15),
                  Expanded(child: _customField("Naziv posla", _naslovController)),
                ],
              ),
              ...
              Container(
                ...
                decoration: BoxDecoration(color: darkFieldColor, borderRadius: BorderRadius.circular(20)),
                child: TextField(
              ...
              GestureDetector(
                ...
                child: Container(
                  width: 80, height: 80,
                  decoration: const BoxDecoration(color: darkFieldColor, shape: BoxShape.circle),
                  child: _isLoading 
                    ? const Padding(padding: EdgeInsets.all(20), child: CircularProgressIndicator(color: Colors.white))
                    : const Icon(Icons.check, size: 50, color: Colors.black),
                ),
              ),
            ],
          ),
  ...
  Widget _customField(String hint, TextEditingController controller, {bool isNumber = false}) {
    return Container(
      padding: const EdgeInsets.symmetric(horizontal: 15),
